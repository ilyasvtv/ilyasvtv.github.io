document.addEventListener('DOMContentLoaded', () => {
    ...
    const addPrice = (e) => {
        ...
        if (elem.children[2].value.trim().length === 0) return;
        if (+elem.children[2].value < 0 && elem.children[1].value.trim().length === 0) return;
    ...
    function checkLotPlace(el) {
        if (+el.children[1].value >= 0 && +el.children[2].value >= 0) {
            ...
            for (let i = totalPriceFields.length - 1; i >= 0; i--) {
                if (totalPriceFields[i].value.trim().length === 0) continue;
    ...
    function addToLocalStorage(el) {
        if (el.children[0].value.trim().length === 0) return;
