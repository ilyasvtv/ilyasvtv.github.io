window.addEventListener('DOMContentLoaded', () => {
    const slides = document.querySelectorAll('.slider__img'),
          slidesWrapper = document.querySelector('.slider__slider'),
          slidesField = document.querySelector('.slider__inner'),
          width = window.getComputedStyle(slidesWrapper).width,
          next = document.querySelector('.slider__next'),
          prev = document.querySelector('.slider__prev'),
          lineElement = document.querySelectorAll('.slider__line .item'),
          sliderInfo = document.querySelectorAll('.slider__info');
    ...
    // Смена фотографии при нажатии на кнопку справа от слайдера
    next.addEventListener('click', () => {
        if (offset == +width.slice(0, width.length - 2) * (slides.length - 1)) {
            offset = 0;
            slideIndex = 0;
            // Смена кнопки
            lineElement[slideIndex].style.background = `black`;
    ...
    function changePhoto(value) {
        ...
        lineElement[value].style.background = `black`;
